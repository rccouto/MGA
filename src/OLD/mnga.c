...
int MNGA(const char *Infile, char *prog, char *gamesspath, char *optstruct, char *rmoutfile, int natoms, int nindiv, int nstep, int percmut, int typmat, int nrotgp, int mutmin, int mutmax, int genafconv, int ngen, int *conv, int gmsver, int ncore, double *energy, double absenrgy, long *nseed, defmol *MPop, Txyz *XYZ, Tcnct *cnct, Tzmat *zmat, Trtt *rtt, Tatom  *rttatm)
{
  char     xyzname[15], datname[15], outname[15], rstname[15], fname[15];
  int      i, j, ip;
  int      gen = ZERO, *pconv, tindiv;
  ...
  do{  
    ...
    /* If it's converged, make only Mutation */
    if ( genconv != 0 ){
      mutpop = nindiv;
      matpop = 0;	
    }
    ...
    for ( i = nindiv; i < 2*nindiv; i++ ) {
      printf("#");fflush(stdout);
      RunProg (i + 1, 0, gmsver, ncore, "I", prog, gamesspath, "Gamess");
      printf("#");fflush(stdout);
	...
	printf("#");fflush(stdout);
      ...
      printf("#");fflush(stdout);
      MNGA_History(i, nindiv, natoms, conv, energy, absenrgy, MPop, XYZ);
      printf("#");fflush(stdout);
    ...
    if (nconv == halfindiv ){
      ReoptPop(prog, gamesspath, xyzname, datname, outname, rstname, natoms, tindiv, nstep, 1, gmsver, ncore, energy, MPop);
      genconv = 0;
    }
