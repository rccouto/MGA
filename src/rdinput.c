  ...
  while ( !feof(fl0) && eof == 1 )
	...
	case 1: /* "**Main" */
	  for ( i = 0; ; i++ )
		...
		nsg = 13;
	      else if ( !strncasecmp(rdaux,"*EEC_Program_path", 17) ) 
		nsg = 14;
		...
		nsg = 21;
	      else if ( !strncasecmp(rdaux,"*OBabel_path", 12) )
		nsg = 22;
	      else if ( !strncasecmp(rdaux,"*Pop_Backup", 11) )
		nsg = 23;
	      else if ( !strncasecmp(rdaux,"*Restart_from_Backup", 20) )
		nsg = 24;
		...
		case 22: /* "*OBabel_path" */
		  fscanf(fl0, "%s", &rdaux);
		  obabelpath = Char_alloc(rdaux);
		  break;
		  
		case 23: /* "*Pop_Backup" */
		  fscanf(fl0, "%s", &rdaux);
		  backup = Char_alloc(rdaux);
		  break;

		case 24: /* "*Restart_from_Backup" */
		  fscanf(fl0, "%s", &rdaux);
		  rstbackup = Char_alloc(rdaux);
		  break;
	...
	case 3: /* "**GA" */
	  for ( i = 0; ; i++ )
		...
		case 30: /* Type_of_GA */
		  ...
		  /*printf ("\n\n\n <<<>>> rdaux: %s <<<>>>  \n\n\n", rdaux); /**/ 
		  if( feof(fl0) || rdaux[1] == '*' ) break;
		  else if ( !strncasecmp(rdaux,"NGA",  3) )  nga = 301;
		  else if ( !strncasecmp(rdaux,"IGA",  3) )  nga = 302;
		  else if ( !strncasecmp(rdaux,"RS",   2) )  nga = 304;
		  
		  switch (nga){
		  case 301: /* "Non-Inclusive Genetic Algorithm (NGA)" */
		    typga = 1;
		    break;
		    
		  case 302: /* "Inclusive Genetic Algorithm (IGA)" */
		    typga = 2;
		    break;
		    
		  case 304: /* "Random Search (RS)" */
		    typga = 4;
		    break;
	...
	case 4: /* "**Force Field" */
	  for ( i = 0; ; i++ )
		...
		nsg = 47;
	      else if ( !strncasecmp(rdaux,"*NCores_Parallel_GAMESS", 23) )
		nsg = 48;
	      else if ( !strncasecmp(rdaux,"*GAMESS_Version_Number", 22) )
		nsg = 49;
	      else if ( !strncasecmp(rdaux,"*Empiric_Force_Field", 20) )
		nsg = 50;
	      else
		nsg = 999;
		...
		case 45: /* "*External_EEC_Programs" */
		  ...
		  prog = (char *) malloc (nos * sizeof(char));/**/
		  
		  fscanf(fl0, "%s", &rdaux);
		  
		  prog = Char_alloc(rdaux);
		  
		  lock = 0;
		  break;

		case 46: /* "*Molecular_charge" */
		  fscanf(fl0, "%s", &rdaux);
		  if ( !strncasecmp(rdaux,"All", 3) ) lock = 1;
		...
		case 48: /* NCores_Parallel_GAMESS */
		  fscanf(fl0, "%d", &ncore);
		  //printf("NCORE %d\n", ncore);
		  break;

		case 49: /* GAMESS_Version_Number */
		  fscanf(fl0, "%d", &gmsver);
		  //printf("GMSVER %d\n", gmsver);
		  break;

		case 50: /* "*Empiric_Force_Field" */
		  fscanf(fl0, "%s", &rdaux);
		  lock = 0;
		  if ( !strncasecmp(rdaux,"All", 3) ) lock = 1;

		  eff = (char *) malloc (nos * sizeof(char));/**/
		  fscanf(fl0, "%s", &rdaux);
